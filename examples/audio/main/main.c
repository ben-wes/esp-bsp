 ...
 * @example
 * https://espressif.github.io/esp-launchpad/?flashConfigURL=https://espressif.github.io/esp-bsp/config.toml&app=audio-
 */
...
/* The recording will be RECORDING_LENGTH * BUFFER_SIZE long (in bytes)
   With sampling frequency 16000 Hz and 16bit mono resolution it equals to ~5.12
   seconds */
#define RECORDING_LENGTH (160)

#define BOARD_PA_EN_PIN 48
...
static void audio_task(void *arg) {
  ...
  const char music_filename[] =
      BSP_SPIFFS_MOUNT_POINT "/16bit_mono_22_05khz.wav";
    ...
    if (xQueueReceive(audio_button_q, &btn_index, portMAX_DELAY) == pdTRUE) {
      ...
      case BSP_BUTTON_VOLDOWN: {
        ...
        int16_t *recording_buffer =
            heap_caps_malloc(BUFFER_SIZE, MALLOC_CAP_DEFAULT);
        ...
        if (fwrite((void *)&recording_header, 1, sizeof(dumb_wav_header_t),
                   record_file) != sizeof(dumb_wav_header_t)) {
        ...
        while (bytes_written_to_spiffs < RECORDING_LENGTH * BUFFER_SIZE) {
          ...
          size_t data_written =
              fwrite(recording_buffer, 1, BUFFER_SIZE, record_file);
      ...
      case BSP_BUTTON_PLAY: {
        ...
        if (fread((void *)&wav_header, 1, sizeof(wav_header), play_file) !=
            sizeof(wav_header)) {
          ESP_LOGW(TAG, "Error in reading file");
          break;
        }
        ESP_LOGI(TAG, "Number of channels: %" PRIu16 "",
                 wav_header.num_channels);
        ESP_LOGI(TAG, "Bits per sample: %" PRIu16 "",
        ...
        while (bytes_send_to_i2s < wav_header.data_size) {
          ...
          size_t bytes_read_from_spiffs =
              fread(wav_bytes, 1, BUFFER_SIZE, play_file);
